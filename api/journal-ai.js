...
다음 JSON 형식으로 반환하세요:
{
  "title": "SEO 최적화 제목 (검색어 포함, 클릭 유도)",
  "description": "메타 설명 160자 이내 (이 글 읽으면 뭘 얻는지 명확히)",
  "category": "FAIRPASS 이야기 | FAIRPASS 활용법 | 운영 사례 | 지속가능한 행사 | 업계 트렌드 | FAIRPASS 소식 중 하나",
...
JSON만 반환하세요. 다른 텍스트 없이.`;
  ...
  try {
    ...
    if (!r.ok) return res.status(500).json({ error: 'Claude API error', detail: await r.text() });
    const result = await r.json();
    const imported = parseJson(result.content[0].text);
