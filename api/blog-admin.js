...
const IMAGES_BASE = 'blog-src/public/blog-images';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'list' && req.method === 'GET') {
      ...
      const dir = `${CONTENT_BASE}/blog-${lang}`;
    ...
    if (action === 'save' && req.method === 'POST') {
      ...
      const commitMsg = message || (sha ? `update: ${path.split('/').pop()}` : `feat: new post ${path.split('/').pop()}`);
    ...
    if (action === 'upload' && req.method === 'POST') {
      ...
      const res2 = await fetch(url, {
        method: 'PUT',
        headers: githubHeaders(),
        body: JSON.stringify({ message: `upload: blog image ${safe}`, content: base64 }),
      });
      const data = await res2.json();
      if (!res2.ok) return res.status(500).json({ error: 'Image upload failed', detail: data });
      // Image served at /blog/blog-images/{safe} after Vercel build
      return res.status(200).json({ success: true, url: `/blog/blog-images/${safe}` });
    }
